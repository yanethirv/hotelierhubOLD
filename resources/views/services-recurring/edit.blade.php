            ...
            <div class="row">
                <div class="col-lg-7 mx-auto">
                    <div class="bg-white rounded-lg shadow-sm p-5">
                        ...
                        <form action="{{ route('subscription-request.update',$serviceRecurring->id) }}" method="POST">
                            @csrf
                            ...
                            <div class="form-group">
                              ...
                              <div class="input-group">
                                  <select class="form-control" name="status">
                                      <option value="active"
                                      @if ($serviceRecurring->status === 'active')
                                          selected
                                      @endif
                                      >{{ __("Active") }}</option>

                                      <option value="inactive"
                                      @if ($serviceRecurring->status === 'inactive')
                                          selected
                                      @endif
                                      >{{ __("Inactive") }}</option>

                                      <option value="process"
                                      @if ($serviceRecurring->status === 'process')
                                          selected
                                      @endif
                                      >{{ __("In Process") }}</option>

                                      <option value="wait"
                                      @if ($serviceRecurring->status === 'wait')
                                          selected
                                      @endif
                                      >{{ __("On Wait") }}</option>
                                  </select>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control" style="height:150px" name="comment" placeholder="comment">{{ $serviceRecurring->comment }}</textarea>
                            </div>
                        </div>
